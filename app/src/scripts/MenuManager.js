...
var defaulttemplate = [{
    ...
    submenu: [{
        ...
        click: function click(_item, focusedWindow) {
...
var MenuManager = /** @class */function () {
    ...
    MenuManager.prototype.pushItem = function (path, itemOptions, indexes) {
        ...
        function insert(arr, value, indexesindex) {
            if (indexes[indexesindex] != null) arr.splice(indexes[indexesindex], 0, created[0]);else arr.push(value);
            return arr;
        }
        ...
        for (var i = 0; i < split.length; i++) {
            ...
            for (var c = 0; c < subs.length; c++) {
                if (subs[c].label == split[i]) {
                    ...
                    if (created[i - 1]) created[i - 1].submenu = subs;else this.template = subs;
                    created.push(subs[i]);
