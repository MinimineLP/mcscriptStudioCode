...
ipcMain.on('get_window', (event:Event, arg) => {
  event.returnValue = mainWindow
});

ipcMain.on('devtools', (event:Event, arg) => {
...
ipcMain.on('get_folder', (event:Event, arg) => {
  if(arg == "get_folder")event.returnValue = dialog.showOpenDialog({properties: ['openFile', 'openDirectory', 'multiSelections']});
});
...
ipcMain.on('', (event:Event, arg) => {
  //@ts-ignore
  event.returnValue = dialog.showMessageBox(mainWindow,{title: <string>arg,buttons: ["ok"]});
});

var alertResponse: any
ipcMain.on('alert', function(event, arg) {
  promptResponse = null
  var promptWindow = new BrowserWindow({width: 200,height: 100,show: false,resizable: false,movable: false,alwaysOnTop: true,frame: false})
  arg.val = arg.val || ''
  const promptHtml = '<label for="val">' + arg + '</label>\
