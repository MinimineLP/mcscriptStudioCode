...
let mainWindow:electron.BrowserWindow = electron.ipcRenderer.sendSync("get_window","get_window");

global.toggleDevTools = function() {electron.ipcRenderer.send("devtools", "toggle")}
global.openDevTools = function() {electron.ipcRenderer.send("devtools", "open")}
global.closeDevTools = function() {electron.ipcRenderer.send("devtools", "close")}
...
manager.loadPlugins(datafolder + "/plugins", function() {
  class MenuActionAPI {
    ...
    trigger(key:string) {
      if(this.listeners[key])this.listeners[key].forEach(function(func: () => any) {
        func();
      });
    }
  }
