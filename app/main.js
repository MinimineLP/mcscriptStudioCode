...
electron_1.ipcMain.on('get_window', function (event, arg) {
    event.returnValue = mainWindow;
});
electron_1.ipcMain.on('devtools', function (event, arg) {
    if (arg == "toggle")
...
electron_1.ipcMain.on('get_folder', function (event, arg) {
    if (arg == "get_folder")
        event.returnValue = electron_1.dialog.showOpenDialog({ properties: ['openFile', 'openDirectory', 'multiSelections'] });
});
...
electron_1.ipcMain.on('', function (event, arg) {
    //@ts-ignore
    event.returnValue = electron_1.dialog.showMessageBox(mainWindow, { title: arg, buttons: ["ok"] });
});
var alertResponse;
electron_1.ipcMain.on('alert', function (event, arg) {
    promptResponse = null;
    var promptWindow = new electron_1.BrowserWindow({ width: 200, height: 100, show: false, resizable: false, movable: false, alwaysOnTop: true, frame: false });
    arg.val = arg.val || '';
...
electron_1.ipcMain.on('alert-response', function (event, arg) {
    if (arg === '') {
        arg = null;
    }
    promptResponse = arg;
...
electron_1.ipcMain.on('prompt-response', function (event, arg) {
    if (arg === '') {
        arg = null;
    }
    promptResponse = arg;
