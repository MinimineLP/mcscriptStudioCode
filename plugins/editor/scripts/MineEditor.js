...
global.reloadEditors = function () {
    ...
    //@ts-ignore
    for (var _a = 0, editors_1 = editors; _a < editors_1.length; _a++) {
        var i = editors_1[_a];
        var id = guid();
    ...
    document.body.addEventListener("keydown", function (e) {
        if (e.ctrlKey)
            document.body.classList.add("ctrl_pressed");
        if (e.shiftKey)
            document.body.classList.add("shift_pressed");
        if (e.altKey)
            document.body.classList.add("alt_pressed");
    });
    document.body.addEventListener("keyup", function (e) {
        if (!e.ctrlKey)
            document.body.classList.remove("ctrl_pressed");
        if (!e.shiftKey)
            document.body.classList.remove("shift_pressed");
        if (!e.altKey)
            document.body.classList.remove("alt_pressed");
    });
    if (editor_config.import_material_icons)
        addStylesheet(editor_config.material_icons);
    if (editor_config.apply_style)
        addStyle(editor_config.style);
    var editorButtons = document.getElementsByClassName('MineEditor-button');
    var setAttribute = function (element) {
...
setInterval(function () {
    for (var _i = 0, toManageEditors_1 = toManageEditors; _i < toManageEditors_1.length; _i++) {
        var i = toManageEditors_1[_i];
        ...
        var lines = 0, words = 0, cols = 0, linesWord = editor_config.language.lines, wordsWord = editor_config.language.words, colsWord = editor_config.language.columns, linesContainer = getFirstSubElementByClass(getFirstSubElementByClass(editor, "MineEditor-bottom"), "MineEditor-lines"), wordsContainer = getFirstSubElementByClass(getFirstSubElementByClass(editor, "MineEditor-bottom"), "MineEditor-words"), colsContainer = getFirstSubElementByClass(getFirstSubElementByClass(editor, "MineEditor-bottom"), "MineEditor-columns");
        ...
        if (lines == 0)
            lines = 1;
        if (lines == 1)
            linesWord = editor_config.language.line;
        if (words == 1)
            wordsWord = editor_config.language.word;
        if (cols == 1)
            colsWord = editor_config.language.column;
    ...
    getAllElementsWithAttribute("tooltip").forEach(function (el) {
        el.addEventListener("mouseenter", function (e) {
            if (this.getAttribute("tooltip")) {
                ...
                var tooltip_1 = document.getElementById("MineEditor_tooltip");
                tooltip_1.innerHTML = this.getAttribute("tooltip");
                tooltip_1.style.top = (position.top + position.height + 3) + "px";
                tooltip_1.style.left = (position.left + (position.width / 2)) + "px";
                tooltip_1.style.display = null;
            }
