...
export class ConsoleContentList {
  ...
  push(content:ConsoleContent):void {
    if(this.size<this.content.length)this.content.splice(0, this.content.length-this.size, content);
  ...
  parseRenderMessageHTML(format:string, content: ConsoleContent):string {
    return format.replace(/%msg%/g, content.getHTML())
                 .replace(/%pre%/g, content.getPrefix())

  }

  parseRenderMessageString(format:string, content: ConsoleContent):string {
    return format.replace(/%msg%/g, content.getString())
                 .replace(/%pre%/g, content.getPrefix())
  }

  renderHTML(max:number=this.size,renderformat=this.renderformat):string {
  ...
  renderString(max:number=this.size,renderformat=this.renderformat):string {
    if(max==0)max=this.size;
    let ret:string = "";
    if(max>=this.content.length)max = this.content.length-1;
    for(let i = max; i>=0; i--) {
  ...
  pipeToPipes(newelement:ConsoleContent):void {
    this.pipes.forEach((pipe:HTMLElement|Function|Stream.Writable) => {
      ...
      else if(pipe instanceof Stream.Writable)pipe._write(newelement.getString(),'utf8',(err)=>{if(err)console.error(err)});
    })
  }
  clear(): void {
    this.content = [];
...
DivConsoleOutStream.prototype._write = function (chunk, encoding, done) {
...
DivConsoleErrorStream.prototype._write = function (chunk, encoding, done) {
