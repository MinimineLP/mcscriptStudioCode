declare class ShortcutbarAPI {id:string;server: ServerApi;constructor(server:ServerApi);addButton(id:string,name:string,icon:string,onclick:any);}
...
export default class Testserver extends Plugin {
  ...
  start(server:ServerApi){
    ...
    api.addButton("start_testserver", "testserver", `<i class="mdi mdi-server-plus"></i>`, () => {
      ...
      if(!activeserver) {
        ...
        walkSync(working_dir).forEach((p:string) => {
          ...
          if(filename.toLowerCase() == "pack.mcmeta") {
            if(Fs.readdirSync(Path.dirname(p)).includes("data")) copyFolderRecursiveSync(Path.dirname(p),`${tmp}/${id}/world/datapacks`);
          }
        });
    ...
    api.addButton("stop_testserver", "stop testserver", `<i class="mdi mdi-server-minus"></i>`, () => {
      $('#testserver').hide();
      ...
      if(!minimized) {
        $("#testserver").animate({
          ...
          $("#testserver").animate({
            ...
            $('#testservericons').children("i").not("#testserverminimize").hide();
            $('#testservername').hide()
        ...
        $('#testservericons').children("i").show();
        $('#testservername').show()
    ...
    Fs.watch(working_dir, (...args:string[]) => {
      ...
      if(args[1].endsWith(".mcfunction")||args[1].endsWith(".mcmeta")||args[1].endsWith(".json"))important = true;
    ...
    this.hasActiveServer = function() {return activeserver};
  }
...
class Server {
  ...
  start() {
    ...
    let javaargs = ['-jar', '-Xms'+config.get("server.minram"), '-Xmx'+config.get("server.maxram")];
    let args = ['nogui'];
    this.spawn = ChildProcess.exec(config.get("javacommand")+' '+javaargs.join(" ")+" "+this.jar.replace(/\\/g,"/")+" "+args.join(" "), {cwd: this.working_dir});
  ...
  execCommand(cmd:string, callback:Function = function(res:any) {Node_Console.log(res)}) {
  let rcon = this.rcon;
  rcon.connect();
...
function setupDrag() {
  ...
  function dragElement(elmnt) {
    ...
    function elementDrag(e) {
      ...
      if(x<1)x=1
      if(y<1)y=1
      if(x+$(elmnt).width()>$(window).width())x = $(window).width()-$(elmnt).width()
      if(y+($(header).height()*2)>$(window).height())y = $(window).height()-($(header).height()*2);
